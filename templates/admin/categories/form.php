...
<section class="admin-panel">
    ...
    <?php if ($isEdit) : ?>
        ...
        <p class="small">
            ...
            <span class="muted">
                <?php if (! empty($category['is_public'])) : ?>
                    — öffentlich, kein Login nötig
                <?php else : ?>
                    — <strong>privat</strong>: ohne Login erfolgt eine Weiterleitung zur Anmeldung
                <?php endif; ?>
            </span>
        </p>
    <?php endif; ?>
    <form method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>" class="admin-form">
        ...
        <label class="field field-inline">
            <input type="checkbox" name="is_public" value="1"<?= (! $isEdit || ! empty($category['is_public'])) ? ' checked' : '' ?>>
            <span>Öffentliche Galerie (ohne Login sichtbar). Deaktiviert = privat, nur für angemeldete Verwalter sichtbar.</span>
        </label>
